...
from .models import QuizProfile, Question, QuestionTour3, AttemptedQuestion, AttemptedQuestionTour3, Subjects, QuizRequest
from .forms import UserLoginForm, RegistrationForm, QuizRequestForm
...
class Play(View):
    ...
    def get_tour_question(self, quiz_profile, subject):
        ...
        print(f'used_test-  {used_questions_pk_test}')
...
class PlayTour3(View):
    ...
    def get_tour3_question(self, quiz_profile, subject):
        ...
        print(f'remaining_tour3-  {remaining_questions_tour3}')
    ...
    def post(self, request, subject_id, *args, **kwargs):
        ...
        right_answer = QuestionTour3.objects.get(pk=question_pk).values_list(
            'right_answer', flat=True)
        ...
        if play_again:
            ...
            try:
                attempted_question = quiz_profile.attempts3tour.select_related('question').get(question__pk=int(question_pk))
                if text_answer == right_answer:
                    subject = Subjects.objects.get(id=subject_id)
                    AttemptedQuestionTour3.objects.create(quiz_profile=quiz_profile,
                                                     question_tour3=attempted_question.question,
                                                     subjects=subject,
                                                     user_anwser=text_answer,
                                                     marks_obtained=attempted_question.marks_obtained,
                                                     is_same=True)
                else:
                    similarity = categorize(right_answer, text_answer)
                    if similarity >= 0.67:
                        subject = Subjects.objects.get(id=subject_id)
                        AttemptedQuestionTour3.objects.create(quiz_profile=quiz_profile,
                                                         question_tour3=attempted_question.question,
